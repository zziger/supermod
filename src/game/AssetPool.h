...
namespace game
{
    struct AssetMeta
    {
        ...
        {
            state.new_usertype<AssetMeta>(sol::no_constructor,
                "notFound", &AssetMeta::notFound,
                "origDir", &AssetMeta::origDir,
                "origName", &AssetMeta::origName,
                "canvasSizeMultiplier", &AssetMeta::canvasSizeMultiplier,
                "loadedManually", &AssetMeta::loadedManually);
        }
    };
    ...
    struct Asset {
        ...
        static void RegisterLuaType(sol::state& state)
        {
            ...
            state.new_usertype<Asset>(sol::no_constructor,
                "name", sol::property(
                    [](const Asset& asset) { return std::string(asset.name); },
                ...
                "width", &Asset::width,
                "height", &Asset::height,
                "texture", sol::property_reinterpret_cast<intptr_t>(&Asset::texture),
                "hasAlpha", &Asset::hasAlpha,
                "isPoolDefault", &Asset::isPoolDefault,
                "format", &Asset::format);
        }
    };
    ...
    public:
        ...
        static void AddToLua(sol::table table)
        {
            ...
            table["loadGameAsset"] = sol::overload_conv<
                Asset* (const fspath&, bool, vector2), Asset* (const string&, bool, vector2),
                Asset* (const fspath&, bool), Asset* (const string&, bool),
                Asset* (const fspath&), Asset* (const string&)
            >([inst](auto... args) { return inst->LoadGameAsset(args...); });

            table["loadGameAssetFromData"] = sol::overload_conv<
                Asset* (const fspath&, bool, vector2), Asset* (const string&, bool, vector2),
                Asset* (const fspath&, bool), Asset* (const string&, bool),
                Asset* (const fspath&), Asset* (const string&)
            >([inst](auto... args) { return inst->LoadGameAssetFromData(args...); });

            table["loadAsset"] = sol::overload_conv<
                Asset* (const fspath&, string, bool, vector2), Asset* (const string&, string, bool, vector2),
                Asset* (const fspath&, string, bool), Asset* (const string&, string, bool),
                Asset* (const fspath&, string), Asset* (const string&, string)
