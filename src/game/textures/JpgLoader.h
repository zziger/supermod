...
public:
	...
	static void AddJpgaData(vector2ui size, std::filesystem::path path, std::vector<byte>& pixelData, const std::vector<byte>& buf, uint32_t bufSize, uint32_t imageSize, bool warn)
	{
	...
	static std::optional<std::vector<byte>> ReadJpgBuf(const byte* buf, size_t bufSize, vector2ui& size, size_t& imageSize)
	{
		try
		{
			...
			if (cinfo.src == nullptr) {
				cinfo.src = static_cast<jpeg_source_mgr*>((*cinfo.mem->alloc_small)(reinterpret_cast<j_common_ptr>(&cinfo), JPOOL_PERMANENT, sizeof(mem_src_mgr)));
			}
	...
	static LPDIRECT3DTEXTURE8 LoadJpga(std::filesystem::path path, vector2ui& size, vector2 canvasSizeMultiplier, bool expectAlpha = false)
	{
		size = { 256, 256 };
