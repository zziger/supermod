...
{
public:
	static LPDIRECT3DTEXTURE8 LoadFromRgbaPixelData(IDirect3DDevice8* device, vector2ui size, vector2 canvasSizeMultiplier, const byte* buf)
	{
		vector2ui surfaceSize {
			static_cast<uint32_t>(std::round(static_cast<float>(size.x) * canvasSizeMultiplier.x)),
			static_cast<uint32_t>(std::round(static_cast<float>(size.y) * canvasSizeMultiplier.y))
		};
		LPDIRECT3DTEXTURE8 sysmemTex = nullptr;
		const auto sysmemRes = device->CreateTexture(surfaceSize.x, surfaceSize.y, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &sysmemTex);

		if (sysmemRes < 0) throw Error(std::format("Не удалось создать текстуру. Ошибка: {}", sysmemRes));
		if (sysmemTex == nullptr) throw Error("Не удалось создать текстуру");
		D3DLOCKED_RECT bits;

		if (FAILED(sysmemTex->LockRect(0, &bits, nullptr, 0))) throw Error("Не удалось создать текстуру. Ошибка: Не удалось заблокировать текстуру для записи");
		...
		if (FAILED(sysmemTex->UnlockRect(0))) throw Error("Не удалось создать текстуру. Ошибка: Не удалось разблокировать текстуру после записи");
