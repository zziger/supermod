...
template <typename TLua, typename TReal, typename TClass>
    requires requires {
        static_cast<TLua>(std::declval<TReal>());
        static_cast<TReal>(std::declval<TLua>());
    }
inline decltype(auto) property_static_cast(TReal TClass::*property)
{
    return sol::property([property](const TClass& obj) { return static_cast<TLua>(obj.*property); },
...
template <typename TLua, typename TReal, typename TClass>
    requires requires {
        reinterpret_cast<TLua>(std::declval<TReal>());
        reinterpret_cast<TReal>(std::declval<TLua>());
    }
inline decltype(auto) property_reinterpret_cast(TReal TClass::*property)
{
    return sol::property([property](const TClass& obj) { return reinterpret_cast<TLua>(obj.*property); },
...
inline table get_package_table(state& state, const std::string& name)
{
    ...
    if (!packages[name].is<table>())
        packages.create_named(name);
