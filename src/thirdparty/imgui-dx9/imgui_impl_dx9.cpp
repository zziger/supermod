...
// EDITED! DO NOT REPLACE WITH ORIGINAL IMPLEMENTATION!
// This implementation was extended to support both IDirect3DTexture8 and IDirect3DTexture9 pointers
// See line 282 for more info
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                try {
                    const auto unknown = static_cast<IUnknown*>(pcmd->GetTexID());
                    ...
                    {
                        texture = nullptr;
                    } else if (SUCCEEDED(unknown->QueryInterface(IID_IDirect3DTexture9, reinterpret_cast<void**>(&texture9))))
                    {
                        texture9->Release();
                        texture = texture9;
                    } else if (SUCCEEDED(unknown->QueryInterface(IID_Direct3DTexture8, reinterpret_cast<void**>(&texture8))))
                    {
                        texture8->Release();
                        if (const auto textureImpl = reinterpret_cast<Direct3DTexture8*>(texture8))
                            texture = textureImpl->GetProxyInterface();
                    }
