...
---@type table<string, { uri: string, path: string }>
local cache             = {}

local function read_manifest(path)
    local success, exists = pcall(fs.exists, path)
...
function ResolveRequire(uri, name)
    if name:byte(1) ~= 0x40 then -- starts from '@'
        return
    end

    local cacheItem = cache[name]
    if cacheItem then
        local results, _ = rpath.findUrisByRequireName(cacheItem.uri, cacheItem.path)
        print("Cache hit", cacheItem.uri, cacheItem.path, util.dump(results))
        return results
    end

    local mod_name, path = name:match("^@([a-zA-Z-_]+)%.(.*)")
    local mod_root = fs.current_path():parent_path()

    if mod_name == nil then
        mod_name = name:sub(2)
        mod_root = mod_root / mod_name

        local manifest = read_manifest(mod_root / "manifest.yml")
        if not manifest then
            return
        end

        path = manifest.main or manifest['lua-script']
        if not path or not path:match("%.lua$") then
            return
        end
    ...
    uri = furi.encode(mod_root:string())
    cache[name] = { uri = uri, path = path }

    local results, _ = rpath.findUrisByRequireName(uri, path)
    print("Cache miss", uri, path, util.dump(results))
    return results
end
