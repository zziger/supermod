...
namespace modloader {
    ...
    std::optional<std::filesystem::path> ModFileResolver::ResolveGameFileInMods(const std::filesystem::path& path) {
        ...
        for (const auto& mod : mods | std::ranges::views::reverse) {
            if (mod->HasFlag(Mod::Flag::INTERNAL) || !mod->IsEnabled()) continue;
    ...
    std::optional<std::filesystem::path> ModFileResolver::GetGameFileInMod(const std::shared_ptr<Mod>& mod, const std::filesystem::path& path) {
        if (!mod || mod->HasFlag(Mod::Flag::INTERNAL) || !mod->IsActive()) return {};
    ...
    std::optional<std::filesystem::path> ModFileResolver::ResolveGameFileInMod(const std::shared_ptr<Mod>& mod, const std::filesystem::path& path) {
    ...
    void ModFileResolver::LoadFiles(const std::filesystem::path& path)
    {
        ...
        for (const auto& file : it) {
            ...
            if (relPath.empty() || relPath.native()[0] == '.') continue;
