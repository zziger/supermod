...
namespace modloader
{
    void ModImplLua::OnEnabled() {
        ...
        package->fenv["findPattern"] = sol::as_function([](const bool cache, const std::string& pat) // todo: depreacate?
        {
            return Memory::Pattern(pat.c_str(), pat.size()).Search(cache).Get<int32_t>();
        });

        package->fenv["safecall"] = sol::as_function([this](const sol::protected_function& f, sol::variadic_args va) -> sol::variadic_results
        {
            auto res = f(va);
            if (!res.valid()) {
        ...
        try
        {
            ...
            package->builtin["events"] = lua.script(sdk->read("library/events.lua"), package->fenv, sol::script_throw_on_error, "built-in events");
            package->builtin["timers"] = lua.script(sdk->read("library/timers.lua"), package->fenv, sol::script_throw_on_error, "built-in timers");

            HookManager::AddLuaIntrinsics(package->fenv);
            package->builtin["memory"] = lua.script(sdk->read("library/memory.lua"), package->fenv, sol::script_throw_on_error, "built-in memory");
            HookManager::RemoveLuaIntrinsics(package->fenv);

            Config::AddLuaIntrinsics(package->fenv, info->GetID());
            package->builtin["config"] = lua.script(sdk->read("library/config.lua"), package->fenv, sol::script_throw_on_error, "built-in config");
    ...
    void ModImplLua::OnDisabled() {
        ...
        if (!weak.expired())
        {
            ...
            spdlog::error("To resolve this issue, make sure that your Lua script does not leave any subscribed functions/values after unloading.");
            spdlog::error("When needed, unsubscribe values in an '_unload' event. See docs for more info.");
            ui::NotificationManager::Notify(std::format("Мод {} не был выгружен полностью. См. консоль для подробностей", info->GetID()));
        }
    }
    ...
    void ModImplLua::Render() {
        ...
        const uint64_t currMs = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
