...
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(local path = fs.path('test'); path:clear(); return path:string())").get<std::string>(), "");
}
...
TEST_F(LuaFilesystemFixture, ShouldRemoveFilename)
{
    ...
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file/'):removeFilename():string())").get<std::string>(), "test/file/");
...
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceFilename('new'):string())").get<std::string>(), "test/new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file/'):replaceFilename('new'):string())").get<std::string>(), "test/file/new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test'):replaceFilename('new'):string())").get<std::string>(), "new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceFilename(fs.path('new')):string())").get<std::string>(), "test/new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file/'):replaceFilename(fs.path('new')):string())").get<std::string>(), "test/file/new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test'):replaceFilename(fs.path('new')):string())").get<std::string>(), "new");
}

TEST_F(LuaFilesystemFixture, ShouldReplaceExtension)
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file.txt'):replaceExtension('new'):string())").get<std::string>(), "test/file.new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file.txt'):replaceExtension('.new'):string())").get<std::string>(), "test/file.new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension('new'):string())").get<std::string>(), "test/file.new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension('.new'):string())").get<std::string>(), "test/file.new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension(''):string())").get<std::string>(), "test/file");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension(fs.path('new')):string())").get<std::string>(), "test/file.new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension(fs.path('.new')):string())").get<std::string>(), "test/file.new");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension(fs.path('')):string())").get<std::string>(), "test/file");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):replaceExtension(fs.path()):string())").get<std::string>(), "test/file");
}
...
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(local path = fs.path('test\\file'); path:makePreferred(); return path:string())").get<std::string>(), "test\\file");
    ASSERT_EQ(lua->safe_script(R"(local path = fs.path('test/file'); path:makePreferred(); return path:string())").get<std::string>(), "test\\file");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test\\file'):makePreferred():string())").get<std::string>(), "test\\file");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file'):makePreferred():string())").get<std::string>(), "test\\file");
...
TEST_F(LuaFilesystemFixture, ShouldAppend)
{
    ...
    ASSERT_EQ(lua->safe_script(R"(return (fs.path('test') / fs.path('file')):string())").get<std::string>(), "test\\file");
    ...
    ASSERT_EQ(lua->safe_script(R"(return (fs.path() / fs.path()):string())").get<std::string>(), "");
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test'):append(fs.path('file')):string())").get<std::string>(), "test\\file");
}
...
TEST_F(LuaFilesystemFixture, ShouldReturnParentPath)
{
    ...
    ASSERT_EQ(lua->safe_script(R"(return fs.path('test/file/'):parentPath():string())").get<std::string>(), "test/file");
...
TEST_F(LuaFilesystemFixture, ShouldChangeCurrentPath)
{
    ...
    ASSERT_EQ(lua->safe_script(R"(fs.currentPath('C:/'); return fs.currentPath():string())").get<std::string>(), "C:\\");
    ASSERT_EQ(std::filesystem::current_path().string(), "C:\\");
    current_path(cwd);

    ASSERT_EQ(lua->safe_script(R"(fs.currentPath(fs.path('C:/')); return fs.currentPath():string())").get<std::string>(), "C:\\");
    ASSERT_EQ(std::filesystem::current_path().string(), "C:\\");
    current_path(cwd);
}
...
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(return fs.tempDirectoryPath():string())").get<std::string>(), std::filesystem::temp_directory_path().string());
}

TEST_F(LuaFilesystemFixture, ShouldReturnAbsolute)
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(return fs.absolute('test'):string())").get<std::string>(), std::filesystem::absolute("test").string());
    ASSERT_EQ(lua->safe_script(R"(return fs.absolute('test/file'):string())").get<std::string>(), std::filesystem::absolute("test/file").string());
    ASSERT_EQ(lua->safe_script(R"(return fs.absolute('test/file/'):string())").get<std::string>(), std::filesystem::absolute("test/file/").string());
...
{
    auto lua = CreateState();
    ASSERT_EQ(lua->safe_script(R"(return fs.relative(fs.currentPath() / "test\\path"):string())").get<std::string>(), "test\\path");
    ASSERT_EQ(lua->safe_script(R"(return fs.relative(fs.currentPath() / "test\\path", fs.currentPath()):string())").get<std::string>(), "test\\path");
    ASSERT_EQ(lua->safe_script(R"(return fs.relative("C:\\test\\path", "C:/"):string())").get<std::string>(), "test\\path");
}
...
TEST_F(LuaFilesystemFixture, ShouldReturnIsEmpty)
{
    ...
    if (exists(tempDir)) remove_all(tempDir);
    ...
    ASSERT_TRUE(lua->safe_script(R"(return fs.isEmpty(fs.tempDirectoryPath() / "supermod-test-empty-dir"))").get<bool>());
    ASSERT_TRUE(lua->safe_script(R"(return fs.isEmpty(fs.tempDirectoryPath() / "supermod-test-empty-file"))").get<bool>());
...
TEST_F(LuaFilesystemFixture, ShouldRemoveFile)
{
    ...
    ASSERT_TRUE(lua->safe_script(R"(return fs.remove(fs.tempDirectoryPath() / "supermod-test-remove-file"))").get<bool>());
...
TEST_F(LuaFilesystemFixture, ShouldRemoveEmptyDirectory)
{
    ...
    ASSERT_TRUE(lua->safe_script(R"(return fs.remove(fs.tempDirectoryPath() / "supermod-test-remove-dir-empty"))").get<bool>());
...
TEST_F(LuaFilesystemFixture, ShouldRemoveDirectory)
{
    ...
    ASSERT_EQ(lua->safe_script(R"(return fs.removeAll(fs.tempDirectoryPath() / "supermod-test-remove-dir"))").get<uint64_t>(), 2);
    ASSERT_FALSE(exists(tempDir));
    ASSERT_FALSE(exists(tempFile));
}
...
TEST_F(LuaFilesystemFixture, ShouldCreateDirectory)
{
    ...
    ASSERT_TRUE(lua->safe_script(R"(return fs.createDirectory(fs.tempDirectoryPath() / "supermod-test-create-dir"))").get<bool>());
...
TEST_F(LuaFilesystemFixture, ShouldCreateDirectories)
{
    ...
    if (exists(tempDirRoot)) remove_all(tempDirRoot);
    if (exists(tempDir)) remove_all(tempDir);

    ASSERT_TRUE(lua->safe_script(R"(return fs.createDirectories(fs.tempDirectoryPath() / "supermod-test-create-dir" / "subdir"))").get<bool>());
    ASSERT_TRUE(exists(tempDir));
    if (exists(tempDirRoot)) remove_all(tempDirRoot);
}
...
TEST_F(LuaFilesystemFixture, ShouldCopyFile)
{
    ...
    if (exists(tempFile)) remove(tempFile);
    if (exists(tempFileCopy)) remove(tempFileCopy);
    std::ofstream file(tempFile);
    file.close();

    lua->safe_script(R"(fs.copy(fs.tempDirectoryPath() / "supermod-test-copy-file", fs.tempDirectoryPath() / "supermod-test-copy-file-copy"))");
...
TEST_F(LuaFilesystemFixture, ShouldCopyDirectory)
{
    ...
    if (exists(tempDir)) remove_all(tempDir);
    if (exists(tempDirCopy)) remove_all(tempDirCopy);
    create_directories(tempDir);
    std::ofstream file(tempFile);
    file.close();

    lua->safe_script(R"(fs.copy(fs.tempDirectoryPath() / "supermod-test-copy-dir", fs.tempDirectoryPath() / "supermod-test-copy-dir-copy"))");
...
TEST_F(LuaFilesystemFixture, ShouldCopyDirectoryRecursively)
{
    ...
    lua->safe_script(R"(fs.copy(fs.tempDirectoryPath() / "supermod-test-copy-dir", fs.tempDirectoryPath() / "supermod-test-copy-dir-copy", { recursive = true }))");
    ASSERT_TRUE(exists(tempDirCopy));
    ASSERT_TRUE(exists(tempDirCopy / "supermod-test-copy-file"));
...
TEST_F(LuaFilesystemFixture, ShouldMoveFile)
{
    ...
    if (exists(tempFile)) remove(tempFile);
    if (exists(tempFileMove)) remove(tempFileMove);
    std::ofstream file(tempFile);
    file.close();

    lua->safe_script(R"(fs.move(fs.tempDirectoryPath() / "supermod-test-move-file", fs.tempDirectoryPath() / "supermod-test-move-file-move"))");
    ASSERT_TRUE(exists(tempFileMove));
    ASSERT_FALSE(exists(tempFile));


    lua->safe_script(R"(fs.rename(fs.tempDirectoryPath() / "supermod-test-move-file-move", fs.tempDirectoryPath() / "supermod-test-move-file"))");
